...
1. **Security first** — Use `crypto/rand` (never `math/rand`). AES-256-GCM for encryption. Never log or print secrets.
2. **Error handling** — Wrap errors with context (`fmt.Errorf("operation: %w", err)`). Never swallow errors silently.
3. **Testing** — Write tests for all new code. `go test ./...` for Go, `cargo test` for Rust. Tests must pass before PR.
...
1. Receive task assignment from leader via `dalcli-leader assign`
2. Create a feature branch from main
3. Implement changes with tests
4. Run `go test ./...` and `cargo test` to verify
