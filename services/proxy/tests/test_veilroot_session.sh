...
case "\$cmd" in
  tool-shell-exports)
    ...
    case "\$tool" in
      codex)
        cat <<EOF
export VEILKEY_PROXY_URL=http://${TEST_PROXY_IP}:18081
export HTTP_PROXY=http://${TEST_PROXY_IP}:18081
export HTTPS_PROXY=http://${TEST_PROXY_IP}:18081
export ALL_PROXY=http://${TEST_PROXY_IP}:18081
export NO_PROXY=127.0.0.1,localhost,.test.internal,.vhost.test
EOF
        ;;
      claude)
        cat <<EOF
export VEILKEY_PROXY_URL=http://${TEST_PROXY_IP}:18084
export HTTP_PROXY=http://${TEST_PROXY_IP}:18084
export HTTPS_PROXY=http://${TEST_PROXY_IP}:18084
export ALL_PROXY=http://${TEST_PROXY_IP}:18084
export NO_PROXY=127.0.0.1,localhost,.test.internal,.vhost.test
EOF
        ;;
      *)
  ...
  tool-proxy-url)
    tool="\$1"
    case "\$tool" in
      codex) echo http://${TEST_PROXY_IP}:18081 ;;
      claude) echo http://${TEST_PROXY_IP}:18084 ;;
      *) exit 2 ;;
    esac
    ;;
...
printf '%s\n' "$out" | grep -q "^VEILKEY_PROXY_URL=http://${TEST_PROXY_IP}:18081$"
