...
func (h *Handler) localFunctionBaseURL() string {
...
func (h *Handler) resolveFunctionRef(ref string) (string, error) {
	...
	req, err := http.NewRequest(http.MethodGet, joinPath(baseURL, agentPathResolve, ref), nil)
	if err != nil {
...
func (h *Handler) renderGlobalFunctionCommand(fn *db.GlobalFunction) (string, error) {
	if fn == nil {
	...
	rendered := functionRunPlaceholderRe.ReplaceAllStringFunc(fn.Command, func(token string) string {
		...
		value, err := h.resolveFunctionRef(spec.Ref)
		if err != nil {
			renderErr = err
			return ""
		}
...
func (h *Handler) buildGlobalFunctionRunEnv(req globalFunctionRunRequest) ([]string, []string, error) {
	env := append(os.Environ(),
		"VEILKEY_LOCALVAULT_URL="+h.localFunctionBaseURL(),
...
func (h *Handler) resolveGlobalFunctionRunTimeout(timeoutSeconds int) time.Duration {
	if timeoutSeconds <= 0 {
...
func (h *Handler) handleGlobalFunctionRun(w http.ResponseWriter, r *http.Request) {
	...
	timeout := h.resolveGlobalFunctionRunTimeout(req.TimeoutSeconds)
