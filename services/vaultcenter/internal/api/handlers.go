...
func (s *Server) SetupAPIRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /api/keycenter/temp-refs", s.requireUnlocked(s.adminHandler.RequireAdminSession(s.handleKeycenterTempRefs)))
	mux.HandleFunc("GET /api/keycenter/temp-refs/{ref}/value", s.requireUnlocked(s.adminHandler.RequireAdminSession(s.handleKeycenterRevealRef)))
