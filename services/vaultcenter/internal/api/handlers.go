...
func (s *Server) SetupAPIRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /api/admin/setup", s.requireTrustedIP(s.requireUnlocked(s.handleAdminSetup)))
	mux.HandleFunc("POST /api/admin/change-password", s.requireUnlocked(s.requireTrustedIP(s.handleAdminChangePassword)))
	mux.HandleFunc("POST /api/admin/login", s.handleAdminLogin)
	mux.HandleFunc("POST /api/admin/logout", s.handleAdminLogout)
	mux.HandleFunc("GET /api/admin/check", s.requireTrustedIP(s.handleAdminCheck))
	...
	mux.HandleFunc("GET /api/registration-tokens/{token_id}/validate", s.requireTrustedIP(s.requireUnlocked(s.handleValidateRegistrationToken)))
