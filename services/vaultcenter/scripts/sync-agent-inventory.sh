...
[[ -n "$db_path" ]] || { echo "Error: could not locate vaultcenter DB path" >&2; exit 1; }
...
for vmid in "${vmids[@]}"; do
  ...
  lxc_exec "$vaultcenter_vmid" "sqlite3 '$db_path' \"UPDATE agents SET vault_hash='${sql_vault_hash}', vault_name='${sql_vault_name}', key_version=${key_version}, secrets_count=${secrets_count}, configs_count=${configs_count}, version=${key_version} WHERE node_id='${sql_node_id}';\""
