...
func (s *Server) preferredHeartbeatIP(r *http.Request) string {
	clientIP, _, _ := net.SplitHostPort(r.RemoteAddr)
	if s.isTrustedIPString(clientIP) {
...
func (s *Server) handleAgentHeartbeat(w http.ResponseWriter, r *http.Request) {
	s.advancePendingRotationsBestEffort()
	...
	if req.IP == "" {
		req.IP = s.preferredHeartbeatIP(r)
	}

	agent, err := s.db.GetAgentByNodeID(nodeID)
	if err == nil {
		if agent.BlockedAt != nil && agent.BlockReason == "key_version_mismatch" && agent.KeyVersion == req.KeyVersion {
			agent, err = s.db.ClearAgentRebind(nodeID)
			if err != nil {
				s.respondError(w, http.StatusInternalServerError, "failed to clear blocked rebind state: "+err.Error())
				return
			}
		}
		if agent.BlockedAt != nil {
			s.respondJSON(w, http.StatusLocked, heartbeatAgentState(agent, "blocked"))
			return
		}
		if agent.RotationRequired {
			if agent.KeyVersion == req.KeyVersion {
				agent, err = s.db.ClearAgentRotation(nodeID)
				if err != nil {
					s.respondError(w, http.StatusInternalServerError, "failed to clear rotation state: "+err.Error())
					return
				}
			} else {
				...
				s.respondJSON(w, http.StatusConflict, resp)
				return
			}
		}
		if agent.RebindRequired && agent.RebindReason == "key_version_mismatch" && agent.KeyVersion == req.KeyVersion {
			agent, err = s.db.ClearAgentRebind(nodeID)
			if err != nil {
				s.respondError(w, http.StatusInternalServerError, "failed to clear rebind state: "+err.Error())
				return
			}
		}
		if agent.RebindRequired {
			agent, err = s.db.AdvanceAgentRebind(nodeID, agent.RebindReason, time.Now().UTC())
			if err != nil {
				s.respondError(w, http.StatusInternalServerError, "failed to update rebind state: "+err.Error())
				return
			}
			if agent.BlockedAt != nil {
				s.respondJSON(w, http.StatusLocked, heartbeatAgentState(agent, "blocked"))
			...
			s.respondJSON(w, http.StatusConflict, heartbeatAgentState(agent, "rebind_required"))
			return
		}
		if agent.AgentHash != "" && agent.KeyVersion != 0 && agent.KeyVersion != req.KeyVersion {
			agent, err = s.db.AdvanceAgentRebind(nodeID, "key_version_mismatch", time.Now().UTC())
			if err != nil {
				s.respondError(w, http.StatusInternalServerError, "failed to update rebind state: "+err.Error())
				return
			}
			if agent.BlockedAt != nil {
				...
				resp["expected_key_version"] = agent.KeyVersion
				resp["provided_key_version"] = req.KeyVersion
				s.respondJSON(w, http.StatusLocked, resp)
				return
			}
	...
	if err := s.db.UpsertAgent(nodeID, req.Label, req.VaultHash, req.VaultName, req.IP, req.Port, req.SecretsCount, req.ConfigsCount, req.Version, req.KeyVersion); err != nil {
		s.respondError(w, http.StatusInternalServerError, "failed to upsert agent: "+err.Error())
		return
	}
	if roleProvided || agent == nil {
		if err := s.db.UpdateAgentRole(nodeID, role); err != nil {
			s.respondError(w, http.StatusInternalServerError, "failed to update agent role: "+err.Error())
			return
		}
	}
	if req.HostEnabled != nil || req.LocalEnabled != nil || roleProvided || agent == nil {
		if err := s.db.UpdateAgentCapabilities(nodeID, role, req.HostEnabled, req.LocalEnabled); err != nil {
			s.respondError(w, http.StatusInternalServerError, "failed to update agent capabilities: "+err.Error())
			return
		}
	}
	if err := s.db.UpdateAgentManagedPaths(nodeID, req.ManagedPaths); err != nil {
		s.respondError(w, http.StatusConflict, "failed to update agent managed_paths: "+err.Error())
		return
	}

	agent, err = s.db.GetAgentByNodeID(nodeID)
	if err != nil {
		s.respondError(w, http.StatusInternalServerError, "failed to get agent: "+err.Error())
		return
	}
	...
	if agent.AgentHash == "" {
		...
		if err != nil {
			s.respondError(w, http.StatusInternalServerError, "failed to generate agent hash")
			return
		}

		agentDEK, err := crypto.GenerateKey()
		if err != nil {
			s.respondError(w, http.StatusInternalServerError, "failed to generate agent DEK")
			return
		}
		...
		if err != nil {
			s.respondError(w, http.StatusInternalServerError, "failed to encrypt agent DEK")
			return
		}

		if err := s.db.UpdateAgentDEK(nodeID, agentHash, encDEK, encNonce); err != nil {
			s.respondError(w, http.StatusInternalServerError, "failed to store agent DEK: "+err.Error())
			return
		}
		...
		resp := map[string]interface{}{
			"status":        "registered",
			"vault_id":      formatVaultID(req.VaultName, req.VaultHash),
