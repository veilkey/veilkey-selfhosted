...
- **KeyCenter** (hub): single control plane per deployment
- **LocalVault** (spoke): one per node, reports to KeyCenter
- **CLI**: operator tool, talks to LocalVault or KeyCenter
...
- Heartbeat to KeyCenter (5-minute interval)
- Execute lifecycle actions under KeyCenter policy (rekey, rebind)
- Report node identity and managed paths
- Serve setup wizard for fresh installs

**Does NOT:**
- Handle plaintext encryption/decryption directly (delegates to KeyCenter)
- Make policy decisions (follows KeyCenter instructions)
...
3. CLI sends plaintext to KeyCenter POST /api/encrypt
4. KeyCenter encrypts with DEK, stores ciphertext
...
2. Operator provides password + KeyCenter URL via wizard or API
3. LocalVault generates salt, derives KEK, creates DEK
4. LocalVault registers with KeyCenter (node_id, vault_hash)
5. KeyCenter records agent, begins accepting heartbeats
...
```
1. KeyCenter schedules rotation for a node
...
5. LocalVault reports completion to KeyCenter
6. KeyCenter updates agent record with new key_version
```
...
| PATCH | `/api/install/keycenter-url` | Set KeyCenter URL |

## Database

Both KeyCenter and LocalVault use SQLite:

- **KeyCenter**: `VEILKEY_DB_PATH` (default: `/opt/veilkey/keycenter/data/veilkey.db`)
