...
3. **Least privilege by default.** LocalVault cannot decrypt secrets on its own — it delegates plaintext operations to KeyCenter. Write operations require trusted IP verification.

4. **Key hierarchy with rotation.** KEK → DEK → ciphertext. DEK rotation is managed by KeyCenter and executed by each LocalVault node independently.
...
| Stale keys after rotation | KeyCenter tracks `key_version` per node, enforces rotation |
...
| Threat | Reason |
|--------|--------|
| Compromised KeyCenter | KeyCenter holds the master key hierarchy — protect it |
...
**Recommendation:** Enable TLS for any deployment where KeyCenter and LocalVault communicate over a network.
...
echo -n 'my-password' > /etc/veilkey/keycenter.password
...
Access the audit log via the admin UI at `http://<keycenter>:10181` or the admin API (`/api/admin/audit/recent`).
